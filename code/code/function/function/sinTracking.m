function [ Out ] = sinTracking( sr, nfft, locP, Fe, offset)
        ...
        while r <= length(listInd)
            ...
            if all(locP(listInd(r) : min(listInd(r) + n_frame, size(locP, 1)), min(c + 1, size(locP, 2))) == 0) && ...
                ...
                while true && c <= size(locP, 2)
                    ...
                    arrayP = max(find(locP(max(listInd(r) - n_frame , 1) : listInd(r), min(c + 1, size(locP, 2))))); 
                    ...
                    if arrayN == arrayP && passo ~= 0 && passo ~= inf
                        ...
                        if countP > countN
                            Out(countRiga, c) = Fe(listInd(r), c);
                            locP(listInd(r), c) = 0;
                        ...
                        if passo == 0
                            Out(countRiga, c) = Fe(listInd(r), c);
                            locP(listInd(r), c) = 0;
                        elseif passo == arrayP && passo ~= inf %&& all(locP(max(listInd(r) - passo, 1) : max(listInd(r) - (passo*2), 1), max(c, 1))) == 0
                            Out(countRiga, c) = Fe(listInd(r), c);
                            locP(listInd(r), c) = 0;
                            listInd(r) = max(listInd(r) - passo, 1);
                        elseif passo == arrayN && passo ~= inf %&& all(locP(min(listInd(r) + passo, size(locP, 1)) : min(listInd(r) + (passo*2),size(locP, 1)), max(c, 1))) == 0
                            Out(countRiga, c) = Fe(listInd(r), c);
                            locP(listInd(r), c) = 0;
                            listInd(r) = min(listInd(r) + passo, size(locP, 1));
                        elseif passo == inf
